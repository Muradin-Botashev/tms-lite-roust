...
namespace Application.BusinessModels.Orders.Triggers
{
    ...
    public class CalcShippingTotalFields : ITrigger<Order>
    {
        private readonly ICommonDataService _dataService;
        private readonly IShippingCalculationService _shippingCalcService;


        public CalcShippingTotalFields(ICommonDataService dataService, IShippingCalculationService shippingCalcService)
        {
            _dataService = dataService;
            _shippingCalcService = shippingCalcService;
        }
        ...
        public void Execute(IEnumerable<EntityChanges<Order>> changes)
        {
            ...
            foreach (var entity in changes.Select(x => x.Entity).Where(x => x.ShippingId != null))
            {
                ...
                _shippingCalcService.RecalculateShipping(shipping, orders);
            }
        }
