...
{
    public class RemoveOrdersFromShippingGenerator : INotificationGenerator
    {
        private readonly ICommonDataService _dataService;

        public RemoveOrdersFromShippingGenerator(ICommonDataService dataService)
        {
        ...
        public IEnumerable<EmailMessage> GenerateEmails(IEnumerable<NotificationEvent> events, string fromEmail, string fromName, string baseSiteUrl)
        {
            ...
            var dataDict = new Dictionary<Guid, Dictionary<string, NotificationOrderDto>>();
            foreach (var @event in events)
            {
                ...
                {
                    var data = JsonConvert.DeserializeObject<NotificationOrdersListDto>(@event.Data);
                    if (data != null && data.Orders != null)
                    {
                        Dictionary<string, NotificationOrderDto> ordersDict;
                        if (!dataDict.TryGetValue(@event.EntityId, out ordersDict))
                        {
                            ordersDict = new Dictionary<string, NotificationOrderDto>();
            ...
            var recipients = _dataService.GetNoTrackingDbSet<User>()
                                         .Where(x => x.IsActive 
                                                    ...
                                                    && x.Notifications.Contains((int)NotificationType.RemoveOrdersFromShipping))
                                         .ToList();
            ...
            foreach (var shipping in shippings)
            {
                ...
                string removedOrders = string.Empty;
                if (dataDict.TryGetValue(shipping.Id, out Dictionary<string, NotificationOrderDto> data))
                {
                    removedOrders = string.Join(", ", data.Values.OrderBy(x => x.OrderNumber).Select(x => $"заявка {x.OrderNumber} на {x.DeliveryAddress}"));
                }
...
<html>
  <body>
    <p>Здравствуйте,</p>
    <p>
      Информируем вас о том, что из перевозки {shipping.ShippingNumber} удалена {removedOrders}.<br/>
    ...
    <p>С уважением, команда TMS Roust</p>
        ...
        {
            return type == NotificationType.RemoveOrdersFromShipping;
        }
    }
}
