...
{
    public class AddOrdersToShippingGenerator : INotificationGenerator
    {
        private readonly ICommonDataService _dataService;

        public AddOrdersToShippingGenerator(ICommonDataService dataService)
        {
        ...
        public IEnumerable<EmailMessage> GenerateEmails(IEnumerable<NotificationEvent> events, string fromEmail, string fromName, string baseSiteUrl)
        {
            ...
            var recipients = _dataService.GetNoTrackingDbSet<User>()
                                         .Where(x => x.IsActive 
                                                    ...
                                                    && x.Notifications.Contains((int)NotificationType.AddOrdersToShipping))
                                         .ToList();
            ...
            foreach (var shipping in shippings)
            {
                ...
                {
                    newOrders = string.Join(", ", data.Values.OrderBy(x => x.OrderNumber).Select(x => $"заявка {x.OrderNumber} на {x.DeliveryAddress}"));
                }
...
<html>
  <body>
    <p>Здравствуйте,</p>
    <p>
      Информируем вас о том, что в перевозку {shipping.ShippingNumber} добавлена {newOrders}.<br/>
    ...
    <p>С уважением, команда TMS Roust</p>
        ...
        {
            return type == NotificationType.AddOrdersToShipping;
        }
    }
}
